...
func main() {
	...
	for {
		...
		case CPU.AND_INY:
			...
			cpu.PC++
			fmt.Printf("%02x (Indirect, Y)", zp)
			addr1, _ := rom.GetWord(uint16(zp))
			addr := addr1 + uint16(cpu.Y)
			v, _ := rom.Get(addr)
			cpu.A &= v
			fmt.Printf("DEBUG: %02x %04x %04x ", zp, addr1, addr)
			cpu.SetStatus(CPU.Negative, CPU.IsNegative(cpu.A))
			cpu.SetStatus(CPU.Zero, cpu.A == 0)
